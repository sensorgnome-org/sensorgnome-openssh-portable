	...
	while (*opts && *opts != ' ' && *opts != '\t') {
		...
		if ((r = opt_flag("restrict", 0, &opts)) != -1) {
			...
			ret->permit_port_forwarding_flag = 0;
                        ret->single_remote_forwarding_port = 0;
		...
		} else if (opt_match(&opts, "single-remote-forwarding-port")) {
                  ret->single_remote_forwarding_port = strtol(opts, (char **) &opts, 0);
...
#define OPTFLAG_AND(x) ret->x = (primary->x == 1) && (additional->x == 1)
	...
	OPTFLAG_AND(single_remote_forwarding_port);
